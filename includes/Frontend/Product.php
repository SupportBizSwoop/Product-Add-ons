...
class Product
{
    ...
    public function show_product_options()
    {
        ...
        if (is_object($product) && $product->get_id() > 0 && !$product instanceof \WC_Product_Grouped) {
            ...
            $add_ons_total_label = get_customize_addon_option('zac_line_item_text', __('Add-ons total', 'product-add-ons-woocommerce'));
                ...
                return $groupAddon
                    ? array_reduce($group['types'], function ($total, $type) use ($groupAddon) {
                        $typeAddon = isset($groupAddon[$type['id']]) ? $groupAddon[$type['id']] : [];
											...
											return array_reduce($type['values'], function ($total, $value) use ($typeAddon, $type) {
													$quantity = self::get_addon_quantity($typeAddon, $value);
													switch ($type['type']) {
															case 'select':
															case 'radio':
																	return ($value['id'] === $typeAddon['value']) ? $total + $quantity * $value['price'] : $total;
															case 'checkbox':
																	return in_array($value['id'], $typeAddon['value']) ? $total + $quantity * $value['price'] : $total;
															case 'text':
															default:
																	return !empty($typeAddon['value'][$value['id']]) ? $total + $quantity * $value['price'] : $total;
													}
											}, $total);
                    }, $total)
                    : $total;
    ...
    public static function add_meta_to_item($product_id, $zaddon, $additional, $item)
    {
        ...
        if ($additional > 0 && get_customize_addon_option('zac_show_additional_line_item', true)) {
            ...
            $item->add_meta_data(
            	get_customize_addon_option('zac_additional_line_item', __('Additional', 'product-add-ons-woocommerce')),
							wc_price($additional),
							true
						);
        }
    ...
    public static function add_item_data($item_data, $cart_item)
    {
        ...
        if ($cart_item['_zaddon_additional'] > 0 && get_customize_addon_option('zac_show_additional_line_item', true)) {
            $item_data[] = [
                'display' => strip_tags(wc_price($cart_item['_zaddon_additional'])),
                'key' => get_customize_addon_option('zac_additional_line_item', __('Additional', 'product-add-ons-woocommerce')),
            ];
        }
        return $item_data;
    }
    ...
    protected static function item_meta($product_id, $zaddon)
    {
        ...
        $zaddon_meta = array_reduce($groups, function ($groups, $group) use ($zaddon) {
            ...
            $types = $groupAddon ? array_reduce($group['types'], function ($types, $type) use ($groupAddon) {
                $typeAddon = isset($groupAddon[$type['id']]) ? $groupAddon[$type['id']] : [];
								$valuesMeta = array_reduce($type['values'], function ($acc, $value) use ($typeAddon, $type) {
										$usedAddon = false;
										switch ($type['type']) {
												case 'select':
												case 'radio':
												...
												default:
														if (!empty($typeAddon['value'][$value['id']])) {
																$acc[$value['id']] = $value['title'] . ' ' . esc_sql($typeAddon['value'][$value['id']]) . ($value['price'] ? ' (' . wc_price($value['price']) . ')' : '');
																$usedAddon = true;
														}
										}
										if ($usedAddon) {
    ...
    public function enqueue_scripts()
    {
        if (is_single() && is_product()) {
            wp_enqueue_script('za_product.js', plugins_url('assets/product.js', \ZAddons\PLUGIN_ROOT_FILE), ['jquery'], '1.1');
            wp_localize_script('za_product.js', 'ZAddons', [
            	'numberOfDecimals' => wc_get_price_decimals(),
						]);
						wp_enqueue_style('za_product.css', plugins_url('assets/product.css', \ZAddons\PLUGIN_ROOT_FILE), '1.0');
        }
    }
    ...
    public function wc_tab_addons($post)
    {
        ...
        <div id="product_zaddons" class="panel woocommerce_options_panel">
            ...
            <div class="options_group">
                <p class="form-field">
                    <label for="_zaddon_disable_global"><?php _e('Disable globals', 'product-add-ons-woocommerce'); ?></label>
                    <input
                            class="checkbox"
                    ...
                    <span class="description">
						<?php _e('Check this box if you want to disable global groups and use the above ones only!', 'product-add-ons-woocommerce'); ?>
					</span>
                </p>
            </div>
        </div>
    ...
    public static function get_addon_quantity($typeAddon, $value) {
				return isset($typeAddon['quantity'][$value['id']]) ? $typeAddon['quantity'][$value['id']] : 1;
    }
